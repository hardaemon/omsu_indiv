...
void f(double(&tabl)[MAX_X1][MAX_X2], int& x1, int& x2, int& number) {
    int pop = 0;
    double per1, per2;
    do {
        for (int j = 0; j < x2; j++) {
    ...
    */
    ...
    do {
        ...
        if (one == 1) {
            ...
            printf("< ------------------------------------------- >\n");
            printf("| 1. Ввод значений | 2. Произвольные значения |\n");
            printf("< ------------------------------------------- >\n\n");
            do {
                printf("Ответ: ");
                scanf("%d", &one);
